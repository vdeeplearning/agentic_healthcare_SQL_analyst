...
@app.get("/schema")
def schema(): _ensure_db(); return {k:sorted(v) for k,v in schema_catalog(settings.db_path).items() if k!="audit_runs"}
@app.get("/metrics")
def metrics(): return {k:v.model_dump() for k,v in METRICS.items()}
@app.post("/analyze",response_model=AnalysisResponse)
def analyze(request:AnalyzeRequest): _ensure_db(); return Analyst(settings.db_path,settings.max_rows,settings.query_timeout_seconds,settings.small_cell_threshold).analyze(request.question,request.api_key,[turn.model_dump() for turn in request.conversation_context])
@app.post("/validate-sql")
def validate(request:SQLRequest): _ensure_db(); return validate_sql(request.sql,settings.db_path).model_dump()
@app.get("/runs")
