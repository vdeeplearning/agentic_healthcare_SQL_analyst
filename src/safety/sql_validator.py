...
def validate_sql(sql: str,path: Path,max_joins:int=8,max_columns:int=20,max_rows:int=1000) -> ValidationReport:
    errors=[]; warnings=[]
    ...
    catalog=schema_catalog(path); cte_names={cte.alias_or_name for cte in tree.find_all(exp.CTE)}
    tables=sorted({t.name for t in tree.find_all(exp.Table) if t.name not in cte_names})
    for table in tables:
        if table not in catalog or table=="audit_runs": errors.append(f"Table is not queryable: {table}")
    ...
    for column in tree.find_all(exp.Column):
        ...
        if column.table:
            ...
            if table in catalog and name not in catalog[table]: errors.append(f"Unknown column: {column.sql()}")
        elif len(tables)==1 and name not in catalog.get(tables[0],set()) and name!="*": errors.append(f"Unknown column: {name}")
